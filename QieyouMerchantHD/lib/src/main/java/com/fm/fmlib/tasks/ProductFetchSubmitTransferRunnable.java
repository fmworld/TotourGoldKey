 ...
 */
public class ProductFetchSubmitTransferRunnable extends TokenCheckedRunnable<TransferEntity> {
    private String item;
    public ProductFetchSubmitTransferRunnable(String item){
        this.item = item;
    }
    @Override
    public TransferEntity doBackground() throws RetrofitError {
        return TourApplication.instance().getmTotour().getmUtilService()
                .transferToH5(UtilService.TransferType.submitOrder.toString(), item, TourApplication.instance().getToken());
    }
