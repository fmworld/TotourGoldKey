 ...
 */
public class ProductFetchListByTagRunnable extends TokenCheckedRunnable<ProductBreEntity> {
    private  String  items;

     public ProductFetchListByTagRunnable(String items){
         this.items = items;
     }
    ...
    public ProductBreEntity doBackground() throws RetrofitError {
        return TourApplication.instance().getmTotour().getProductService().fetchProduct(TourApplication.instance().getToken(), items);
    }
